...
local function GSUICJ_fake_script() -- Fake Script: StarterGui.ScreenGui.OpenButton.Abrir-MenuLocalScript
	...
	-- Encontra o ScreenGui que contém o OpenButton
	local screenGui = script.Parent.Parent  -- Ajuste para acessar o ScreenGui que contém o OpenButton
	local spiFrame = screenGui:FindFirstChild("SPIFrame")  -- Acessa o SPIFrame1 pelo nome
	
	-- Variável para verificar se está arrastando
	local isDragging = false
	
	-- Detecta quando o jogador começa a arrastar o botão
	script.Parent.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			isDragging = false
	
			-- Conecta o evento de movimentação do mouse
			local conn
			conn = input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					conn:Disconnect()  -- Desconecta o evento de movimentação do mouse quando terminar de arrastar
				end
			end)
		end
	end)
	
	-- Detecta a movimentação do mouse enquanto arrasta
	...
	-- Conecta a função de abertura ao evento MouseButton1Click do OpenButton
	script.Parent.MouseButton1Click:Connect(function()
		-- Verifica se o botão não está sendo arrastado
		if isDragging then
			isDragging = false
			return
		end
		...
		if spiFrame then
			-- Torna o SPIFrame1 visível
			spiFrame.Visible = true
		else
			warn("SPIFrame1 não encontrado.")
		end
	end)
...
local function UUMW_fake_script() -- Fake Script: StarterGui.ScreenGui.OpenButton.Arrasta-MenuLocalScript
	...
	-- Variáveis para armazenar a posição inicial do mouse e do botão
	...
	-- Função para atualizar a posição do botão enquanto arrasta
	...
	-- Detecta quando o jogador solta o botão para impedir a abertura do menu
	button.MouseButton1Click:Connect(function()
		if isDragging then
			isDragging = false
			return
		end
		-- Coloque aqui o código para abrir o menu, se necessário
	end)
...
local function YNIFZ_fake_script() -- Fake Script: StarterGui.ScreenGui.LocalScript
	...
	-- Acesso direto à ScreenGui se estiver dentro da ScreenGui
	local screenGui = script.Parent
	
	if screenGui then
		print("ScreenGui encontrado:", screenGui.Name)
	else
		print("Não foi possível encontrar a ScreenGui")
	end
	...
	print(code)  -- Exibe o código gerado no output
...
local function PTTAW_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.SPIFrameMenu.MainButton.LocalScript
	...
	local mainButton = script.Parent  -- Este script está dentro do MainButton
	
	-- Função para lidar com o clique no MainButton
	local function onMainButtonClick()
		-- Encontra o ScreenGui que contém o MainFrame, HatchFrame, e SettingsFrame
		...
		if mainFrame then
			-- Torna o MainFrame visível
			mainFrame.Visible = true
		else
			warn("MainFrame não encontrado.")
		end
	
		-- Esconde os outros frames se estiverem visíveis
		if hatchFrame then
	...
	-- Conecta a função onMainButtonClick ao evento MouseButton1Click do MainButton
...
local function EDFW_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.SPIFrameMenu.HatchButton.LocalScript
	...
	local hatchButton = script.Parent  -- Este script está dentro do HatchButton
	
	-- Função para lidar com o clique no HatchButton
	local function onHatchButtonClick()
		-- Encontra o ScreenGui que contém o HatchFrame, MainFrame, e SettingsFrame
		...
		if hatchFrame then
			-- Torna o HatchFrame visível
			hatchFrame.Visible = true
		else
			warn("HatchFrame não encontrado.")
		end
	...
	-- Conecta a função onHatchButtonClick ao evento MouseButton1Click do HatchButton
...
local function FFKK_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.SPIFrameMenu.SettingsButton.LocalScript
	...
	local settingsButton = script.Parent  -- Este script está dentro do SettingsButton
	
	-- Função para lidar com o clique no SettingsButton
	local function onSettingsButtonClick()
		-- Encontra o ScreenGui que contém o SettingsFrame, MainFrame, e HatchFrame
		...
		if settingsFrame then
			-- Torna o SettingsFrame visível
			settingsFrame.Visible = true
		else
			warn("SettingsFrame não encontrado.")
		end
	...
	-- Conecta a função onSettingsButtonClick ao evento MouseButton1Click do SettingsButton
...
local function WPSKGSM_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.SPIFrameMenu.MainFrame.Main-ContainerFrame.ADAutoComboioButton.Espandimento-LocalScript
	...
	local button = script.Parent  -- Assume que este script está dentro do ADAutoComboioButton
	local containerFrame = button.Parent  -- Assume que Main-ContainerFrame é o pai do botão
	
	-- Armazenar o tamanho original do botão
	local originalSize = button.Size
	local isCovering = false
	
	-- Função para cobrir ou descobrir o Main-ContainerFrame
	...
	-- Conectar a função ao evento MouseButton1Click do botão
...
local function VQLBXRG_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.CloseButton.LocalScript
	...
	-- Encontra o SPIFrame1 que está no mesmo nível do CloseButton
	local spiFrame = script.Parent.Parent  -- Ajuste para acessar o SPIFrame1
	
	-- Conecta a função de fechamento ao evento MouseButton1Click do CloseButton
	script.Parent.MouseButton1Click:Connect(function()
		-- Verifica se o SPIFrame1 foi encontrado
		if spiFrame then
			-- Torna o SPIFrame1 invisível
...
local function VAXZH_fake_script() -- Fake Script: StarterGui.ScreenGui.SPIFrame1.Arrasta-FrameLocalScript
	...
	-- Variáveis para armazenar a posição inicial do mouse e do frame
	local dragging = false
	local dragStart = nil
	local startPos = nil
	local userInputService = game:GetService("UserInputService")
	...
	-- Função para atualizar a posição do frame enquanto arrasta
	local function update(input)
		local delta = input.Position - dragStart
		frame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
	end
	
	-- Detecta quando o jogador começa a arrastar o frame
	...
	-- Detecta quando o jogador solta o frame para impedir a abertura de menus ou ações indesejadas
